...
void wfdb_addtopath( char *s )
{
	...
	if ( p < s ) {
		return;    /* argument did not contain a path component */
	}
...
void wfdb_setirec( const char *p )
{
	...
	for ( r = p; *r; r++ )
		if ( *r == DSEP ) {
			p = r + 1;    /* strip off any path information */
		}
#ifdef MSDOS
		else if ( *r == ':' ) {
			p = r + 1;
...
static long nf_get_range( netfile *nf, long startb, long len, char *rbuf )
{
	...
	if ( len > avail ) {
		len = avail;    /* limit request to available bytes */
	}
...
static size_t nf_fread( void *ptr, size_t size, size_t nmemb, netfile *nf )
{
	...
	if ( nf == NULL || ptr == NULL || bytes_requested == 0 ) {
		return ( ( size_t )0 );
	}
	...
	if ( bytes_requested > page_size && page_size ) {
		bytes_requested = page_size;
	}
...
FINT WEP( int nParameter )
{
	if ( nParameter == WEP_SYSTEM_EXIT || WEP_FREE_DLL ) {
		wfdbquit();
	}	/* Always close WFDB files before shutdown or
